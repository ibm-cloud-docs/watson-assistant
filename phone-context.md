...
{: caption="Voice context variables set by the dialog or actions" caption-side="bottom"}

### Example
{: #phone-context-variables-set-by-dialog-example}
...
| `sip_custom_invite_headers` | object | A JSON object with key or value pairs that define SIP headers that are pulled from the initial SIP `INVITE` request and passed to the {{site.data.keyword.conversationshort}} service (for example, `{"Custom-Header1": "123"}`). |
...
{: caption="Context variables set by the phone channel" caption-side="bottom"}
...
### Example
{: #phone-context-variables-set-by-phone-channel-example}
