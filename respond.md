...
1. In the **Duration** field, enter the length of time for the pause in `seconds`.
    ...
    The duration must not exceed `60 seconds`. In addition, you can enter the decimals of `second` in the **Duration** field to configure the pause response in `milliseconds`. For example, if you enter `10.50 seconds`, it means the response pauses for `10500 milliseconds`. Customers are typically willing to wait about 8 seconds for someone to enter a response. 
