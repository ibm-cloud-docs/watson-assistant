...
To configure the web chat, complete the following steps:

1. On the ![Integrations icon](images/integrations-icon.png) **Integrations** page, find the **Web chat** tile.

1. Click **Open**. The **Edit channel** window opens.

1. In the **Environment** field, select either **Draft** or **Live**, depending on which environment you want to edit web chat for. Then click **Confirm**.

    A preview pane is displayed that shows you what the web chat will look like when it is embedded in a web page. If a message is displayed that starts with, `There is an error`, it means you haven't added any actions to your assistant yet. After you add an action, you can test the conversation from the preview pane.
    {: tip}
