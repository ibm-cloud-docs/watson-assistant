...
# Calling a service after processing a message for {{site.data.keyword.cpd_full_notm}}
{: #webhook-post-cp4d}
...
For more information about related features and details, see the following resources:
 ...
 - [Calling a service before processing a message for {{site.data.keyword.cpd_full_notm}}](/docs/watson-assistant?topic=watson-assistant-webhook-pre-cp4d)
...
This section covers the procedure to define, test, and remove post-message webhooks for {{site.data.keyword.cpd_short}}.

- [**Webhook configuration**](#post-webhook-config-cp4d)
...
This example shows how to translate a message response back to the customer's language. It works only if you perform the steps in [Example 2](/docs/watson-assistant?topic=watson-assistant-webhook-pre-cp4d#pre-example-2-cp4d) to define a pre-message webhook that translates the original message into English.
